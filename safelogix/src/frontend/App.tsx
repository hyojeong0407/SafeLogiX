...
function App() {
  ...
  const handleLogin = async () => {
    ...
    try {
      const response = await fetch('http://192.168.200.104:8000/login', {
        method: 'POST',
        headers: {
  ...
  if (!isAuthenticated) {
    return (
      <div className="login-container" style={{ textAlign: 'center', marginTop: '100px' }}>
        ...
        />
        <h2 style={{ color: '#333' }}>현장 작업자 로그인</h2>
        <div style={{ marginTop: '20px' }}>
          <input 
            ...
            onKeyDown={(e) => e.key === 'Enter' && handleLogin()} // 엔터키로도 로그인 되게 추가
            style={{ padding: '10px', fontSize: '16px', borderRadius: '5px', border: '1px solid #ccc' }}
          />
          <button 
            onClick={handleLogin} 
            style={{ padding: '10px 20px', marginLeft: '10px', fontSize: '16px', cursor: 'pointer', backgroundColor: '#0056b3', color: 'white', border: 'none', borderRadius: '5px' }}
          >
            접속하기
          </button>
        </div>
